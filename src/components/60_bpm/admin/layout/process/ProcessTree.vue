...
import Subprocess from '@/components/60_bpm/common/process/nodes/SubprocessNode.vue'
...
export default {
  ...
  methods: {
    ...
    insertNode (type, parentNode) {
      ...
      switch (type) {
        ...
        case 'CONDITIONS':
          ...
          break
        case 'INCLUSIVES':
          this.insertInclusiveNode(parentNode)
          break
        case 'CONCURRENTS':
    ...
    insertTaskNode (parentNode) {
      ...
      this.$set(parentNode.children, 'props', deepcopy(DefaultProps.TASK_PROPS))
    },
      ...
      this.$set(parentNode.children, 'branchs', [
        {
          ...
          parentId: parentNode.children.id,
          type: 'INCLUSIVE',
          props: deepcopy(DefaultProps.INCLUSIVE_PROPS),
          name: '包容条件1',
          children: {}
        }, {
          id: this.getRandomId(),
    ...
    addBranchNode (node) {
      if (node.branchs.length < 8) {
        node.branchs.push({
          ...
          props: this.isConditionNode(node) ? deepcopy(DefaultProps.CONDITION_PROPS) : this.isInclusiveNode(node) ? deepcopy(DefaultProps.INCLUSIVE_PROPS) : {},
