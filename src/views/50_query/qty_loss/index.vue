    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:dataJson.constants_program.P_SCHEDULE_QTY_LOSS,query:{goods_name:scope.row.goods_name, goods_code:scope.row.goods_code,batch: dataJson.searchForm.batch, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.qty === null?'': isZhongLin ? formatNumber3(scope.row.qty) : formatNumberTwo(scope.row.qty,true,2) }}</a></router-link>
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<script>
import { getLossQtListApi, getLossQtListSumApi, exportApi, exportAllApi } from '@/api/50_query/qty_loss/loss_all'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      getLossQtListApi(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      getLossQtListSumApi(this.dataJson.searchForm).then(response => {
