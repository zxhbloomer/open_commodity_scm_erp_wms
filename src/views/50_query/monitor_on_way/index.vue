<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_MONITOR_WAY:EXPORT'"
    ...
    <div class="div-sum">
      <div class="right">
        <span class="count-title">合计发货数量：</span><span class="count-data">{{ dataJson.sumData.out_qty == null ? '0': isZhongLin ? formatNumber3(dataJson.sumData.out_qty) : formatNumberTwo(dataJson.sumData.out_qty,true,2) }}</span>
        <span class="count-title">合计收货数量：</span><span class="count-data">{{ dataJson.sumData.in_qty == null ? '0': isZhongLin ? formatNumber3(dataJson.sumData.in_qty) : formatNumberTwo(dataJson.sumData.in_qty,true,2) }}</span>
        <span class="count-title">合计在途数量：</span><span class="count-data">{{ dataJson.sumData.out_sum_way_qty == null ? '0': isZhongLin ? formatNumber3(dataJson.sumData.out_sum_way_qty) : formatNumberTwo(dataJson.sumData.out_sum_way_qty,true,2) }}</span>
      </div>
    </div>
    <el-table
      ref="multipleTable"
    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:dataJson.constants_program.P_MONITOR,query:{p2:scope.row.id, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.code }}</a></router-link>
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
      ...
      >
        <template v-slot="scope">
          {{ scope.row.out_way_qty === null?'': isZhongLin ? formatNumber3(scope.row.out_way_qty) : formatNumberTwo(scope.row.out_way_qty,true,2) }}
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<script>
import { getMonitorListSumApi, getMonitorListApi } from '@/api/50_query/qty_loss/monitor_loss'
import { exportApi, exportAllApi } from '@/api/50_query/qty_loss/monitor_in_transit'
...
export default {
  ...
  created () {
    ...
    if (this.$route.query.sku_name !== null && this.$route.query.sku_name !== undefined) {
      this.dataJson.searchForm.sku_name = this.$route.query.sku_name
    }
    ...
    if (this.$route.query.customer_name !== null && this.$route.query.customer_name !== undefined) {
      this.dataJson.searchForm.customer_name = this.$route.query.customer_name
    }
  ...
  methods: {
    ...
    getDataList () {
      ...
      getMonitorListApi(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      getMonitorListSumApi(this.dataJson.searchForm).then(response => {
