<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_GRAIN_OUT_DETAILS:EXPORT'"
...
<script>
import { getGrainOutListSumApi, getGrainOutPageListApi, exportGrainOutListApi } from '@/api/50_query/processingLedger'
    ...
    'dataJson.searchForm': {
      ...
      handler () {
        if (this.dataJson.searchForm.daterange === null) {
          this.dataJson.searchForm.time_start = null
          this.dataJson.searchForm.time_end = null
        }
      }
    }
  },
  ...
  methods: {
    ...
    handleSearch () {
      ...
      if (this.dataJson.searchForm.daterange !== null && this.dataJson.searchForm.daterange !== undefined && this.dataJson.searchForm.daterange !== '') {
        this.dataJson.searchForm.time_start = this.dataJson.searchForm.daterange[0]
        this.dataJson.searchForm.time_end = this.dataJson.searchForm.daterange[1]
      }
      this.getDataList()
      // 清空选择
    ...
    getDataList () {
      ...
      getGrainOutPageListApi(this.dataJson.searchForm).then(response => {
      ...
      getGrainOutListSumApi(this.dataJson.searchForm).then(response => {
    ...
    handleExportAllData () {
      ...
      // 开始导出
      exportGrainOutListApi(this.dataJson.searchForm).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
    ...
    handleExportSelectionData () {
      ...
      // 开始导出
      exportGrainOutListApi({ ids: selectionJson }).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
