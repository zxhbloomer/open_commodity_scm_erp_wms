<template>
  <div class="app-container">
    ...
    <el-button-group v-if="searchsetting.visible">
      <el-button
        v-permission="'P_LOSS_OUT:EXPORT'"
    ...
    <div class="div-sum">
      <div class="right">
        <span class="count-title">合计销售数量：</span><span class="count-data">{{ dataJson.sumData.qty == null?'0':dataJson.sumData.qty>0? isZhongLin ? formatNumber3(dataJson.sumData.qty) : formatNumberTwo(dataJson.sumData.qty,true,2) :0 }}</span>
        <span class="count-title">合计出库数量：</span><span class="count-data">{{ dataJson.sumData.actual_count == null?'0':dataJson.sumData.actual_count>0? isZhongLin ? formatNumber3(dataJson.sumData.actual_count) : formatNumberTwo(dataJson.sumData.actual_count,true,2) :0 }}</span>
      </div>
    </div>
    <el-table
      ref="multipleTable"
    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:constants_program.P_OUT,query:{goods_name:scope.row.goods_name, goods_code:scope.row.goods_code, owner_name:scope.row.owner,supplier_name: scope.row.supplier_name,batch:dataJson.searchForm.batch, type: '5', fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.actual_count === null?'': isZhongLin ? formatNumber3(scope.row.actual_count) : formatNumberTwo(scope.row.actual_count,true,2) }}</a></router-link>
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<script>
import { getOutListSumApi, getOutListApi, exportApi, exportAllApi } from '@/api/50_query/qty_loss/out'
...
export default {
  ...
  created () {
    ...
    // }
    // 如果类型不是副产品稻壳, 查询需要剔除 副产品稻壳 商品
    if (this.$route.query.goods_type !== '副产品') {
      this.dataJson.searchForm.showTips = true
    }
  ...
  methods: {
    ...
    getDataList () {
      ...
      getOutListApi(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      getOutListSumApi(this.dataJson.searchForm).then(response => {
