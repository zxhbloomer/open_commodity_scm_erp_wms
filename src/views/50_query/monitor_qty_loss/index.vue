<template>
  <div class="app-container">
    ...
    <el-button-group v-if="searchsetting.visible">
      <el-button
        v-permission="'P_MONITOR_QTY_LOSS:EXPORT'"
    ...
    >
      <div class="right">
        <span class="count-title">合计已发货数量：</span><span class="count-data">{{ dataJson.sumData.out_qty == null ? '0': isZhongLin ? formatNumber3(dataJson.sumData.out_qty) :formatNumberTwo(dataJson.sumData.out_qty,true,2) }}</span>
        <span class="count-title">合计已收货数量：</span><span class="count-data">{{ dataJson.sumData.in_qty == null ? '0': isZhongLin ? formatNumber3(dataJson.sumData.in_qty) : formatNumberTwo(dataJson.sumData.in_qty,true,2) }}</span>
    ...
    <el-table
      ...
      :default-sort="{prop: 'schedule_code', order: 'ascending'}"
      style="width: 100%"
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ scope.row.qty_loss === null?'': isZhongLin ? formatNumber3(scope.row.qty_loss) :formatNumberTwo(scope.row.qty_loss,true,2) }}
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<script>
import { getMonitorListApi, getMonitorListSumApi, exportApi, exportAllApi } from '@/api/50_query/qty_loss/monitor_loss'
