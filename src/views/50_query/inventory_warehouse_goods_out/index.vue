<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_INVENTORY_OUT:EXPORT'"
    ...
    <div class="div-sum">
      <div class="right">
        <span class="count-title">合计出库库存：</span><span class="count-data">{{ dataJson.sumData.qty == null?'0': isZhongLin ? formatNumber3(dataJson.sumData.qty) : formatNumberTwo(dataJson.sumData.qty,true,2) }}</span>
      </div>
    </div>
    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:constants_program.P_OUT,query:{goods_name:scope.row.goods_code, warehouse_id:scope.row.warehouse_id,batch: dataJson.searchForm.batch,warehouse_type_ll:scope.row.warehouse_type, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.qty === null?'': isZhongLin ? formatNumber3(scope.row.qty) : formatNumberTwo(scope.row.qty,true,2) }}</a></router-link>
        </template>
...
<script>
import { queryReportOutList, queryReportOutListSum, exportApi, exportAllApi } from '@/api/50_query/inventory_warehouse_goods_out'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      queryReportOutList(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      queryReportOutListSum(this.dataJson.searchForm).then(response => {
