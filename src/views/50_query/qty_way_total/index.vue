<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_QTY_TOTAL:EXPORT'"
    ...
    <el-table
      ...
      :default-sort="{prop: 'goods_prop', order: 'ascending'}"
      style="width: 100%"
    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:constants_program.P_QTY_ON_WAY,query:{goods_prop:scope.row.goods_prop,batch: dataJson.searchForm.batch, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.qty === null?'': isZhongLin ? formatNumber3(scope.row.qty) : formatNumberTwo(scope.row.qty,true,2) }}</a></router-link>
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
import {
  exportOnWayApi,
  exportOnWayAllApi,
  getWayQtyAndInventorListApi, getWayQtyAndInventorListSumApi
} from '@/api/50_query/qty_loss/qty_total'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      getWayQtyAndInventorListApi(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      getWayQtyAndInventorListSumApi(this.dataJson.searchForm).then(response => {
    ...
    handleExportAllData () {
      ...
      // 开始导出
      exportOnWayAllApi(this.dataJson.searchForm).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
    ...
    handleExportSelectionData () {
      ...
      // 开始导出
      exportOnWayApi(selectionJson).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
