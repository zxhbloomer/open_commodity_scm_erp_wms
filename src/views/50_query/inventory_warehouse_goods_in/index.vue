<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_INVENTORY_IN:EXPORT'"
    ...
    <div class="div-sum">
      <div class="right">
        <span class="count-title">合计入库库存：</span><span class="count-data">{{ dataJson.sumData.qty == null?'0': isZhongLin ? formatNumber3(dataJson.sumData.qty) : formatNumberTwo(dataJson.sumData.qty,true,2) }}</span>
    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:constants_program.P_IN,query:{goods_name:scope.row.goods_code, warehouse_id:scope.row.warehouse_id, batch: dataJson.searchForm.batch, un_type: scope.row.warehouse_type === '2' ? '5' : null, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.qty === null?'': isZhongLin ? formatNumber3(scope.row.qty) : formatNumberTwo(scope.row.qty,true,2) }}</a></router-link>
        </template>
      </el-table-column>
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:constants_program.P_IN,query:{goods_name:scope.row.goods_code, warehouse_id:scope.row.warehouse_id, batch: dataJson.searchForm.batch, type: '7', fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.return_qty === null?'': isZhongLin ? formatNumber3(scope.row.return_qty) : formatNumberTwo(scope.row.return_qty,true,2) }}</a></router-link>
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<script>
import { queryReportInList, queryReportInListSum, exportApi, exportAllApi } from '@/api/50_query/inventory_warehouse_goods_in'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      queryReportInList(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      queryReportInListSum(this.dataJson.searchForm).then(response => {
