<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_SCHEDULE_WAY:EXPORT'"
    ...
    <div class="div-sum">
      <div class="right">
        <span class="count-title">合计在途数量：</span><span class="count-data">{{ dataJson.sumData.qty_loss == null ? '0': isZhongLin ? formatNumber3(dataJson.sumData.qty_loss) : formatNumberTwo(dataJson.sumData.qty_loss,true,2) }}</span>
      </div>
    </div>
    <el-table
      ref="multipleTable"
    ...
    >
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:dataJson.constants_program.P_MONITOR,query:{schedule_code:scope.row.schedule_code, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.schedule_code }}</a></router-link>
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
      ...
      >
        <template v-slot="scope">
          <router-link :to="{name:dataJson.constants_program.P_MONITOR_WAY,query:{sku_name:scope.row.sku_name, sku_code:scope.row.sku_code, schedule_code:scope.row.schedule_code,customer_name:scope.row.customer_name,batch:dataJson.searchForm.batch, fullpath:true}}"><a href="javascript:void(0)">{{ scope.row.out_qty === null?'': isZhongLin ? formatNumber3(scope.row.out_qty) : formatNumberTwo(scope.row.out_qty,true,2) }}</a></router-link>
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
...
<script>
import { getScheduleListSumApi, getScheduleListApi } from '@/api/50_query/qty_loss/schedule_loss'
import { exportApi, exportAllApi } from '@/api/50_query/qty_loss/schedule_in_transit'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      getScheduleListApi(this.dataJson.searchForm).then(response => {
      ...
      // 查询合计信息
      getScheduleListSumApi(this.dataJson.searchForm).then(response => {
    ...
    handleExportSelectionData () {
      ...
      this.dataJson.multipleSelection.forEach(function (value, index, map) {
        selectionJson.push({ 'id_str': value.id_str })
      })
      // 开始导出
