<template>
  <div class="app-container">
    ...
    >
      <el-form-item>
        <select-supplier
          ...
          placement="left"
          @onReturnDataName="(val) => dataJson.searchForm.client_name = val"
      ...
      <!--        <el-date-picker-->
      <!--          v-model="dataJson.searchForm.e_dt"-->
      <!--          value-format="yyyy-MM-dd"-->
      <!--          type="date"-->
      <!--          style="width: 100%"-->
      <!--          clearable-->
    ...
    <el-button-group>
      <el-button
        v-permission="'P_RICEHUL_OUT_DETAIL:EXPORT'"
    ...
    <el-table
      ...
      :default-sort="{prop: 'client_name', order: 'ascending'}"
      style="width: 100%"
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ scope.row.gross_weight === null?'': isZhongLin ? formatNumber3(scope.row.gross_weight) : formatNumberTwo(scope.row.gross_weight,true,2) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
      ...
      >
        <template v-slot="scope">
          {{ scope.row.tare_weight === null?'': isZhongLin ? formatNumber3(scope.row.tare_weight) : formatNumberTwo(scope.row.tare_weight,true,2) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
...
<script>
import { getRiceHullPageListApi, exportRiceHullListApi } from '@/api/50_query/processingLedger'
...
export default {
  components: { SelectSupplier, Pagination, SelectDict },
  ...
  methods: {
    ...
    getDataList () {
      ...
      getRiceHullPageListApi(this.dataJson.searchForm).then(response => {
    ...
    doResetSearch () {
      ...
      this.dataJson.searchForm.reset1 = !this.dataJson.searchForm.reset1
    },
    // 获取row-key
    getRowKeys (row) {
      return row.id
    ...
    handleExportAllData () {
      ...
      // 开始导出
      exportRiceHullListApi(this.dataJson.searchForm).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
    ...
    handleExportSelectionData () {
      ...
      // 开始导出
      exportRiceHullListApi({ ids: selectionJson }).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
