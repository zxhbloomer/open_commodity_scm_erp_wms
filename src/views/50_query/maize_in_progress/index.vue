<template>
  <div class="app-container">
    ...
    <el-button-group>
      <el-button
        v-permission="'P_MAIZE_IN_PROGRESS:EXPORT'"
    ...
    <div class="div-sum">
      <div class="right">
        <span class="count-title">合计入库净重：</span><span class="count-data">{{ dataJson.sumData.actual_count == null?'0': isZhongLin ? formatNumber3(dataJson.sumData.actual_count) : formatNumberTwo(dataJson.sumData.actual_count,true,2) }}</span>
      </div>
    </div>
    <el-table
      ref="multipleTable"
...
<script>
import { getMaizePageListApi, getMaizePageListSumApi, exportMaizeListApi } from '@/api/50_query/processingLedger'
...
export default {
  ...
  methods: {
    ...
    getDataList () {
      ...
      getMaizePageListApi(this.dataJson.searchForm).then(response => {
      ...
      getMaizePageListSumApi(this.dataJson.searchForm).then(response => {
    ...
    handleExportAllData () {
      ...
      this.settings.loading = true
      // 开始导出
      exportMaizeListApi(this.dataJson.searchForm).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
    ...
    handleExportSelectionData () {
      ...
      // 开始导出
      exportMaizeListApi({ ids: selectionJson }).then(response => {
        this.settings.loading = false
      }).finally(() => {
        this.settings.loading = false
      })
