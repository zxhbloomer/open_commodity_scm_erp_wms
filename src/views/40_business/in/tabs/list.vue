  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <el-form-item label="是否去除稻壳杂质">
          <select-dict
            v-model="dataJson.searchForm.remove_impurity"
            :para="CONSTANTS.DICT_B_IN_REMOVE_IMPURITY"
            init-placeholder="去除稻壳杂质"
          />
        </el-form-item>

        <el-row>
          <el-col :span="12">
            <el-form-item label="入库时间起止">
              <el-date-picker
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ scope.row.primary_quantity == null?'':isZhongLin ? formatNumber3(scope.row.primary_quantity) :formatNumber(scope.row.primary_quantity,true,4) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
      ...
      >
        <template v-slot="scope">
          {{ scope.row.actual_weight == null?'':isZhongLin ? formatNumber3(scope.row.actual_weight) :formatNumber(scope.row.actual_weight,true,4) }}
        </template>
      </el-table-column>
      <el-table-column
        :sort-orders="settings.sortOrders"
      ...
      >
        <template v-slot="scope">
          {{ scope.row.car_count == null?'':formatNumber(scope.row.car_count) }}
        </template>
      </el-table-column>
      <el-table-column
        :sort-orders="settings.sortOrders"
...
import { getTodoCountApi, getListSumApi, exportAllBInApi, exportBInApi, importBInApi, getListApi, auditApi, submitApi, returnApi, cancelApi, syncApi, syncAllApi, getCountApi, cancelAuditApi, cancelReturnApi } from '@/api/40_business/in/in'
...
import cancelConfirmDialog from '@/views/40_business/in/tabs/dialog/cancelconfirm'
...
export default {
  components: { SelectSupplier, Pagination, SelectDicts, SelectDict, SelectConsignor, SelectWarehouse, SimpleUpload, cancelConfirmDialog },
  ...
  methods: {
    ...
    setWatch () {
      ...
      _this.watch.unwatch_tempJson = _this.$watch('dataJson.multipleSelection', (newVal, oldVal) => {
        ...
        _this.dataJson.multipleSelection.forEach(function (value, index, array) {
          if (value.status !== constants_type.DICT_B_IN_STATUS_ZD) {
            if (value.status === constants_type.DICT_B_IN_STATUS_BH) {
              // 审核驳回状态可提交
              _this.settings.btnStatus.showSubmit = true
            } else {
              // 非制单状态不可提交
          ...
          if (value.status !== constants_type.DICT_B_IN_STATUS_YTJ && value.status !== constants_type.DICT_B_IN_STATUS_CANCEL_BEING_AUDITED) {
            // 非已提交/作废待审核状态不可审核/驳回
            _this.settings.btnStatus.showAudit = false
            _this.settings.btnStatus.showReject = false
          }
    ...
    handleCurrentChange (row) {
      ...
      if (this.dataJson.currentJson.id !== undefined) {
        ...
        if (this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_ZF || this.dataJson.currentJson.monitor_in_id !== null ||
          this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_CANCEL_BEING_AUDITED || this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_YTJ) {
          // 已作废的状态不可重复作废, 监管任务来的不可作废
          this.settings.btnStatus.showCancel = false
        }
        // 已作废的不可修改
        if (this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_ZF || this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_YTJ ||
          this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_CANCEL_BEING_AUDITED || this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_TG) {
    ...
    handleAuditSelectionData () {
      ...
      this.dataJson.multipleSelection.forEach(function (value, index, array) {
        ...
        if (value.status === constants_type.DICT_B_IN_STATUS_CANCEL_BEING_AUDITED) {
          cancelAuditSelectionJson.push({ 'id': value.id })
        } else if (value.status === constants_type.DICT_B_IN_STATUS_YTJ) {
    ...
    handleCancel () {
      ...
      // 状态为制单或驳回的数据才弹窗
      if (this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_ZD || this.dataJson.currentJson.status === constants_type.DICT_B_IN_STATUS_BH) {
