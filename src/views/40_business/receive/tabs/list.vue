<template>
  <div class="app-container">
      ...
      >
        ...
        <el-form-item label="">
          <select-dict
            ...
            :para="CONSTANTS.DICT_B_RECEIVE_BUSINESS_TYPE"
        ...
        <el-form-item>
          <select-dicts
            ...
            :para="CONSTANTS.DICT_B_RECEIVE_STATUS"
      ...
      >
        ...
        <el-form-item label="出库类型">
          <select-dicts
            ...
            :para="CONSTANTS.DICT_B_RECEIVE_TYPE"
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ isZhongLin ? formatNumber3(scope.row.actual_count) : formatNumber(scope.row.actual_count,true,4) }}
        </template>
      </el-table-column>
      <el-table-column
        sortable="custom"
      ...
      <!--      >-->
      <!--        <template v-slot="scope">-->
      <!--          {{ isZhongLin ? formatNumber3(scope.row.actual_weight) : formatNumber(scope.row.actual_weight,true,4) }}-->
      <!--        </template>-->
      <!--      </el-table-column>-->
      <!--      <el-table-column-->
      <!--        :sort-orders="settings.sortOrders"-->
...
import {
  ...
  syncAllApi,
  syncApi
} from '@/api/40_business/receive/receive'
...
export default {
  components: { Pagination, SelectDict, SelectDicts, SelectConsignor, SelectWarehouse, cancelConfirmDialog, SelectSupplier },
  ...
  created () {
    ...
    if (this.$route.query.start_time !== null && this.$route.query.start_time !== undefined) {
      ...
      this.dataJson.searchForm.todo_status = '2'
      this.dataJson.searchForm.active_tabs_index = '2'
      this.dataJson.searchForm.status_list.push(constants_type.DICT_B_RECEIVE_STATUS_TG)
      this.dataJson.tabs.active = '2'
    }
    // 报表业务起始时间
  ...
  methods: {
    setWatch () {
      ...
      _this.watch.unwatch_tempJson = _this.$watch('dataJson.multipleSelection', (newVal, oldVal) => {
        ...
        _this.dataJson.multipleSelection.forEach(function (value, index, array) {
          if (value.status !== constants_type.DICT_B_RECEIVE_STATUS_ZD) {
            if (value.status === constants_type.DICT_B_RECEIVE_STATUS_BH) {
              // 审核驳回后可再次提交
              _this.settings.btnStatus.showSubmit = true
            } else {
              // 非制单状态不可提交
          ...
          if (value.status !== constants_type.DICT_B_RECEIVE_STATUS_YTJ && value.status !== constants_type.DICT_B_RECEIVE_STATUS_CANCEL_BEING_AUDITED) {
            // 非已提交状态不可审核/驳回
            _this.settings.btnStatus.showAudit = false
            _this.settings.btnStatus.showReject = false
          }

          if (value.status !== constants_type.DICT_B_RECEIVE_STATUS_TG) {
            // 非审核通过状态不可完成
            _this.settings.btnStatus.showFinish = false
          }
        })
    ...
    handleCurrentChange (row) {
      ...
      if (this.dataJson.currentJson.id !== undefined) {
        ...
        if (this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_ZF || this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_EXPIRES || this.dataJson.currentJson.monitor_out_id !== null ||
          this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_CANCEL_BEING_AUDITED || this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_YTJ) {
          // 已作废的状态不可重复作废, 来自监管任务的不可作废, 已过期的状态不可作废, 已提交的不可作废
          this.settings.btnStatus.showCancel = false
        }
        // 选中已提交、审核通过、作废、作废审核状态中的出库单，修改按钮不亮
        if (this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_ZF || this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_YTJ ||
          this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_TG || this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_CANCEL_BEING_AUDITED) {
    ...
    handleAuditSelectionData () {
      ...
      this.dataJson.multipleSelection.forEach(function (value, index, array) {
        ...
        if (value.status === constants_type.DICT_B_RECEIVE_STATUS_CANCEL_BEING_AUDITED) {
          cancelAuditSelectionJson.push({ 'id': value.id })
        } else if (value.status === constants_type.DICT_B_RECEIVE_STATUS_YTJ) {
    ...
    handleCancel () {
      // 状态为制单或驳回的数据才弹窗
      if (this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_ZD || this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_BH) {
        let msg = '该单据为制单状态，点击作废将直接更新状态为作废，请谨慎操作。确定要作废吗？'
        if (this.dataJson.currentJson.status === constants_type.DICT_B_RECEIVE_STATUS_BH) {
