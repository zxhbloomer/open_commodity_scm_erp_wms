<template>
  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item>
        <select-dict
          ...
          :para="CONSTANTS.DICT_B_ALLOCATION_STATUS"
          init-placeholder="状态"
      ...
      <el-form-item>
        <el-select
          v-model.trim="dataJson.searchForm.out_warehouse_id"
          placeholder="请选择调出仓库"
      ...
      <el-form-item>
        <el-select
          v-model.trim="dataJson.searchForm.in_warehouse_id"
          placeholder="请选择调入仓库"
          :loading="settings.loading"
          loading-text="拼命加载..."
          clearable
      ...
      <el-form-item>
        <el-select
          ...
          :placeholder="isPlaceholderShow('请选择调出货主')"
          :disabled="isViewModel"
          filterable
          clearable
        >
      ...
      <el-form-item>
        <el-select
          ...
          :placeholder="isPlaceholderShow('请选择调入货主')"
          :disabled="isViewModel"
          filterable
          clearable
        >
    ...
    >
      ...
      >
        <el-form-item label="请选择调出委托方">
          <select-consignor
            v-model.trim="dataJson.searchForm.out_consignor_name"
            :placeholder="isPlaceholderShow('请选择调出委托方')"
            :type="CONSTANTS.DICT_CONSIGNOR"
            :reset="settings.reset1"
            @closeParentDialog="handleDialogClose"
            @onReturnData="handleConsignorReturnDataOut"
            @keyup.enter.native="handleSearch"
          />
        </el-form-item>
        <el-form-item label="请选择调入委托方">
          <select-consignor
            v-model.trim="dataJson.searchForm.in_consignor_name"
            :placeholder="isPlaceholderShow('请选择调入委托方')"
            :type="CONSTANTS.DICT_CONSIGNOR"
            :reset="settings.reset1"
            @closeParentDialog="handleDialogClose"
            @onReturnData="handleConsignorReturnDataIn"
            @keyup.enter.native="handleSearch"
          />
        </el-form-item>
    ...
    <el-button-group>
      ...
      <el-button
        v-permission="'P_INVENTORY_ALLOCATE:ADD'"
        type="primary"
      ...
      <el-button
        v-permission="'P_INVENTORY_ALLOCATE:INFO'"
      ...
      >查看</el-button>
      <el-button
        v-permission="'P_INVENTORY_ALLOCATE:AUDIT'"
      ...
      >审核</el-button>
      <el-button
        v-permission="'P_INVENTORY_ALLOCATE:DELETE'"
        :disabled="!settings.btnStatus.showEnable"
        type="primary"
        icon="el-icon-close"
        :loading="settings.loading"
...
import { getListApi, auditApi, deleteApi } from '@/api/40_business/allocate/allocate'
...
export default {
    ...
    return {
      ...
      dataJson: {
        ...
        searchForm: {
          ...
          in_consignor_id: null,
          out_consignor_id: null
        },
        todoSearchForm: {},
        // 分页控件的json
  ...
  watch: {
    ...
    'dataJson.multipleSelection': {
      ...
      handler (newVal, oldVal) {
        const screenKeys = ['daterange', 'contract_no', 'in_consignor_id', 'out_consignor_id']
  ...
  methods: {
    ...
    handleConsignorReturnDataIn (val) {
      this.dataJson.searchForm.in_consignor_id = val.id
    },
    handleConsignorReturnDataOut (val) {
      this.dataJson.searchForm.out_consignor_id = val.id
    },
    handleSupplierReturnData (val) {
      this.dataJson.searchForm.supplier_id = val.id
    },
    handleOwnerReturnData (val) {
      this.dataJson.searchForm.in_owner_id = val.id
    },
    // 弹出框关闭
    handleDialogClose () {
