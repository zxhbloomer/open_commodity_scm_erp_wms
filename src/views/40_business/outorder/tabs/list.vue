<template>
  <div class="app-container">
      ...
      >
        ...
        <el-form-item>
          <select-dicts
            ...
            :para="CONSTANTS.DICT_B_OUT_ORDER_STATUS"
            :filter-para="['-1']"
    ...
    <el-button-group v-if="searchsetting.visible">
      ...
      >新增</el-button>
      <el-button
        v-permission="'P_OUT_ORDER:UPDATE'"
      ...
      >修改</el-button>
      <el-button
        v-permission="'P_OUT_ORDER:EXPORT'"
      ...
      >导出</el-button>
      <el-button
        v-permission="'P_OUT_ORDER:DELETE'"
      ...
      >删除</el-button>
      <el-button
        v-permission="'P_OUT_ORDER:SEND_OUT_PLAN'"
        :disabled="!settings.btnShowStatus.showSendOutPlan"
        type="primary"
        icon="el-icon-s-management"
        :loading="settings.loading"
        @click="handleSendOutPlan"
      >下推出库计划</el-button>
      <el-button
        v-permission="'P_OUT_ORDER:INFO'"
    ...
    >
      ...
      >
        <template v-slot="scope">
          {{ scope.row.out_actual_count == null?'0': isZhongLin ? formatNumber3(scope.row.out_actual_count) : formatNumber(scope.row.out_actual_count,true,4) }}
        </template>
      </el-table-column>
...
<script>
import { getListApi, getListSumApi, exportApi, deleteApi } from '@/api/40_business/outorder/outorder'
import resizeMixin from '@/views/40_business/inorder/tabs/mixin/listResizeHandlerMixin'
import Pagination from '@/components/Pagination'
import elDragDialog from '@/directive/el-drag-dialog'
import editDialog from '@/views/40_business/outorder/dialog/edit'
...
export default {
  components: { Pagination, editDialog, SelectSupplier, SelectDicts, SelectDict },
  ...
  methods: {
    ...
    handleCurrentChange (row) {
      ...
      if (this.dataJson.currentJson.id !== undefined) {
        ...
        if (this.dataJson.currentJson.source_type !== '1') {
          this.settings.btnShowStatus.showUpdate = false
          this.settings.btnShowStatus.showSendOutPlan = false
        } else {
          this.settings.btnShowStatus.showUpdate = true
          this.settings.btnShowStatus.showSendOutPlan = true
        }
    ...
    handleSendOutPlan () {
      this.$router.push({
        path: '/out/plan', query: { p4: this.dataJson.currentJson.id, sku_code: this.dataJson.currentJson.sku_code, fullpath: true }
      })
    },
    // 查看
    handleView () {
      ...
      // 更新
      const operate_tab_data = {
        operate_tab_info: { show: true, name: '新增出库订单' },
        canEdit: true,
