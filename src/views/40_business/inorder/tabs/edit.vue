      ...
      >
        ...
        >
          ...
          <el-descriptions-item label="订单编号">
            {{ dataJson.tempJson.order_no == null || dataJson.tempJson.order_no == ''?'系统自动生成':dataJson.tempJson.order_no }}
          ...
          <el-descriptions-item>
            ...
            >
              <el-input
                v-model.trim="dataJson.tempJson.ship_name"
                clearable
                show-word-limit
                :maxlength="dataJson.inputSettings.maxLength.ship_name"
                :placeholder="isPlaceholderShow('请输入')"
                :disabled="isViewModel"
              />
            </el-form-item>
        ...
        />
        <br>
        <el-button-group v-show="!isViewModel">
          <el-button
            type="primary"
        ...
        >
          ...
          >
            <template v-slot="scope">
              {{ scope.row.in_actual_count == null? '-': isZhongLin ? formatNumber3(scope.row.in_actual_count) : formatNumber(scope.row.in_actual_count,true,4) }}
            </template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
          ...
          >
            <template v-slot="scope">
              {{ scope.row.price == null? '': formatDate(scope.row.delivery_date) }}
            </template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
...
import { updateApi, insertApi, getApi } from '@/api/40_business/inorder/inorder'
import ownerDialog from '@/views/30_wms/owner/dialog/dialog'
import supplierDialog from '@/views/30_wms/customer/dialog/dialog'
import BusinessSelect from '@/views/30_wms/businesstype/component/businessSelect'
// import businessDialog from '@/views/30_wms/businesstype/dialog/dialog'

export default {
  components: { InputSearch, SelectDict, ownerDialog, goodsDialog, supplierDialog, BusinessSelect },
      ...
      settings: {
        ...
        rules: {
          ...
          supplier_name: [
            ...
            { validator: this.supplier_name_validator, trigger: 'change' }
          ],
          // 销售企业
          owner_name: [
            // { required: true, message: '请选择采购企业', trigger: 'change' }
  ...
  methods: {
    ...
    // 供应商校验
    supplier_name_validator (rule, value, callback) {
      if (this.dataJson.tempJson.supplier_name === null || this.dataJson.tempJson.supplier_name === undefined) {
